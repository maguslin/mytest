...
Texture2DMS<float4>	g_colorTextureMs : register(t1);
...
{   
	...
	{
		output.depth = sampleMinDepth(g_sampleCountMsaa, location,	g_depthTextureMs);
	}
	else
	{
		output.depth = sampleMaxDepth(g_sampleCountMsaa, location, g_depthTextureMs);
	}

	output.color = sampleColor(g_sampleCountMsaa, location, g_colorTextureMs);
